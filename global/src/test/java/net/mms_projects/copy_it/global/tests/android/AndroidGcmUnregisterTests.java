...
public class AndroidGcmUnregisterTests {

    @Test(
            expected = JsonParseException.class,
            timeout = 500
    )
    public void gcmUnregisterWithAInvalidJsonResponse() throws Throwable {
        Android.GcmUnregister endpoint = new Android.GcmUnregister();

        Response response = null;
        try {
            response = new MockOAuthResponse(
                    new URL("http://api.copyit.mmsdev.org/1/android/unregister"), 200, "Invalid JSON yay!"
            );
